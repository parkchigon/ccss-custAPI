...
{	
	...
	@Value("#{config['custClient.thirdHostName']}")
	...
	private static int esIdxServer = 0;
	...
	{		
		...
		{				
			...
			{
				System.out.println("##########API_ID_CUST_LOG_SERVICE_DETAIL resData:"+resData.toString());
				System.out.println("##########API_ID_CUST_LOG_SERVICE_DETAIL reqCusJSON:"+reqCusJSON.toString());
			...
			else 
			{
				...
				System.out.println("########## reqCusJSON:"+reqCusJSON.toString());
	...
	public RestClient checkReq() {
		...
		map.put("thirdHostName", thirdHostName);

		for (int i = 0; i < 3; i++) {
			if (esIdxServer == 0) {
				esStrServer = "firstHostName";
			} else if (esIdxServer == 1) {
				esStrServer = "secondHostName";
			} else if (esIdxServer == 2) {
				esStrServer = "thirdHostName";
			}
			Map<String, Object> resultMap = restClient((String) map.get(esStrServer));
